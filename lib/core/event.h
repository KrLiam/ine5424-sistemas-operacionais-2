...
struct TransmissionComplete : public Event {
    ...
    TransmissionComplete(UUID transmission_uuid, const MessageIdentity& id, const SocketAddress& source, const SocketAddress& remote_address);
};
...
struct PipelineCleanup : public Event {
    ...
    PipelineCleanup(const MessageIdentity& id, const SocketAddress& destination);
};
...
struct HeartbeatReceived : public Event {
    ...
    HeartbeatReceived(Node& remote_node, Packet& packet);
};
